...
resource "aws_eip" "nat_eip_us_east_1c" {
  ...
  tags = merge(
    map(
      "Name", "${local.name_prefix}-nat-us-east-1c"
    ),
    local.common_tags
  )
...
resource "aws_nat_gateway" "nat_gw_us_east_1c" {
  allocation_id = aws_eip.nat_eip_us_east_1c.id
  subnet_id     = aws_subnet.public_us_east_1c.id

  tags = merge(
    map(
      "Name", "${local.name_prefix}-nat-gw-us-east-1c"
    ),
    local.common_tags
  )
